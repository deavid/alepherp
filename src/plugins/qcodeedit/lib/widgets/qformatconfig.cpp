...
QFormatConfig::QFormatConfig(QWidget *w)
	: QWidget(w), m_autonomous(false), m_currentScheme(0), ui(new Ui::FormatConfig)
{
	ui->setupUi(this);
	...
	connect(ui->m_table, SIGNAL( itemSelectionChanged() ),
			ui->m_table, SLOT  ( clearSelection() ) );
	
}

QFormatConfig::~QFormatConfig()
{
	delete ui;
}
...
void QFormatConfig::apply()
{
	if ( m_currentScheme )
	{
		...
		for ( int i = 0 ; i < n; ++i )
		{
			...
			cp = qobject_cast<QSimpleColorPicker*>(ui->m_table->cellWidget(i, 7));
			if ( cp )
				fmt.foreground = cp->color();
			
			cp = qobject_cast<QSimpleColorPicker*>(ui->m_table->cellWidget(i, 8));
			if ( cp )
				fmt.background = cp->color();
			
			cp = qobject_cast<QSimpleColorPicker*>(ui->m_table->cellWidget(i, 9));
			if ( cp )
